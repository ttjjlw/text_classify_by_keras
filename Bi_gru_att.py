...
from keras.layers import Dense,Dropout,Conv1D,MaxPooling1D,Flatten,merge,Input,Embedding,GRU,BatchNormalization,Bidirectional,Permute,multiply
...
LEARN_RATE = 0.001
FILTER_SIZE=3
model_name='Bi_gru_att/'
WEIGHTS_PATH = "save_model/Bi_gru_att/29_19_28_False_orderly_post_weights_best.h5"  # 载入模型时用，注意每次手动更新加载最优模型
...
def attention_3d_block(inputs):
    ...
    a = Dense(PADDING_SENTENCE_LENGTH, activation='softmax')(a)
...
def creat_model(sentence_length,vocal_size,embeding_dim,embed_matrix):
    ...
    drop=Dropout(DROUPOUT_RATE)(attention)
...
def load_model(weights_path, model_frame_path):
    ...
    with open(model_frame_path, 'r') as f:
        model_json = f.read()
    #因为有自定义类，所以需要添加custom_objects={'AttentionLayer':AttentionLayer}，否则在导入模型时会找不到AttentionLayer
    model = model_from_json(model_json,custom_objects={'AttentionLayer':AttentionLayer})
