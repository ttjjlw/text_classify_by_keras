...
def Gettime():
    ...
    return d
T=Gettime()
ONLY_TEST=False #训练模式时当True时直接载入模型进行验证集上验证
FILTER_SIZE_LIST=[1,2,3,4,5]
FILTERS_NUM=100
...
model_name='Textcnn_2/'
WEIGHTS_PATH="save_model/Textcnn_2/29_20_23_True_orderly_post_weights_best.h5"  #载入模型时用，注意每次手动更新加载最优模型
...
TRAINABLE=True   #embeddings是否可以训练
...
MODE='train'    #训练模式还是测试测试集模型[train or predict]
##------------------------------------------------函数定义--------------------------------------------
...
def creat_model(filter_size_list,sentence_length,learn_rate,vocal_size,embeding_dim,embed_matrix):
    ...
    embed=Dropout(rate=EMBED_DROP_RATE)(embed)
    pooled_out=[]
    for filter_size in filter_size_list:
        cov1=Conv1D(filters=FILTERS_NUM,kernel_size=filter_size,strides=1,padding='valid',activation='relu')(embed)
        normal_cov1=BatchNormalization()(cov1)
        cov2=Conv1D(FILTERS_NUM,kernel_size=filter_size,strides=1,padding='valid',activation='relu')(normal_cov1)
        normal_cov2=BatchNormalization()(cov2)
        pool=MaxPooling1D(pool_size=sentence_length-2*filter_size+2)(normal_cov2)
        pool_flatten=Flatten()(pool)
        pooled_out.append(pool_flatten)
    merge=concatenate(pooled_out,axis=1)
    out1 = Dropout(DROUPOUT_RATE)(merge)
    hid=Dense(HIDDEN_SIZE,activation='relu',kernel_constraint=maxnorm(4))(out1)
    normal_hid=BatchNormalization()(hid)
    out2=Dropout(DROUPOUT_RATE)(normal_hid)
    output=Dense(NUM_CLASSES,activation='softmax',kernel_constraint=maxnorm(4))(out2)
...
if MODE=='predict':
    ...
    for method in method_set:
        ...
        if method=='reverse':
            for line in test_set:
                line.reverse()
        for tru in truncat:
            test_set_padding=pad_sequences(maxlen=1500,sequences=test_set,truncating=tru)
            print('predict.....')
            logits=predict_test_set(test_set_padding,weights_path=WEIGHTS_PATH,
    ...
    try:
        save_path=save_dir+T+'textcnn.csv'
        df.to_csv(save_path,index=None)
        print('结果保存路径:{}'.format(save_path))
    except:
        df.to_csv('result/28_15_09_textcnn.csv',index=None)
    print('finish!')
